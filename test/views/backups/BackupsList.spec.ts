...
describe('BackupsList', () => {
    it('renders ServiceListTable component', ({ annotate }) => {
        annotate('Backups List: verifies the ServiceListTable child component is rendered in the DOM');
        const wrapper = mount(BackupsList, mountOptions);
        expect(wrapper.find('[data-testid="service-list-table"]').exists()).toBe(true);
    });

    it('sets page heading', ({ annotate }) => {
        annotate('Backups List: verifies the component mounts successfully and sets the page heading');
        const wrapper = mount(BackupsList, mountOptions);
