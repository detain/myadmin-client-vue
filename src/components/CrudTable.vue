...
const validPageLimits = computed(() => {
    return pageLimits.value.filter((limit) => limit === -1 || limit <= totalPages.value);
});
const page = ref(1);
...
const headerButtons = ref(["<a class='btn btn-primary btn-sm printer-hidden' href='domain_order');' data-toggle=\"tooltip\" title=\"Order Domain Registrations\"><i class='fa fa-shopping-cart'></i> Order</a>"]);
const titleButtons = ref(['<a class=\'btn btn-info active btn-sm\' onclick=\'crud_search(this, [["domain_status","=","active"]]);\'>Active</a>', '<a class=\'btn btn-info btn-sm\' onclick=\'crud_search(this, [["domain_status","in",["pending","pending-setup","pend-approval"]]]);\'>Pending</a>', '<a class=\'btn btn-info btn-sm\' onclick=\'crud_search(this, [["domain_status","in",["canceled","expired"]]]);\'>Expired</a>', "<a class='btn btn-info btn-sm' onclick='crud_search(this, []);'>All</a>"]);
const labels = ref({
...
const filters = ref({
    domain_hostname: [{ type: 'string', value: '<a href="index.php?choice=none.view_domain&id=%domain_id%" data-container="body" title="View Domain Registrations">%value%</a>' }],
});
const rows = ref([
...
const rowButtons = ref(['<button type="button" alt="View Domain Registrations" class="btn btn-primary btn-xs printer-hidden" onclick="window.location=\'index.php?choice=none.view_domain&id=\'+get_crud_row_id(this);" title="View Domain Registrations" data-toggle="tooltip" tooltip="View Domain Registrations"><i class="fa fa-fw fa-cog"></i></button>']);
...
function crud_submit_handler(what, that) {
    ...
    $.ajax({
        ...
        success: function (html) {
            ...
            const theModal = jQuery('#' + what + 'Modal .error_message');
            theModal.html('');
            if (html.substring(0, 4) == 'true') {
                theModal.html('<div style="margin: 15px; text-align: center;"><i class="fa fa-spinner fa-spin fa-2x"></i> <span style="margin-left: 10px;font-size: 18px;">Redirecting</span><div>');
                if (html.length == 4) {
                ...
                theModal.html('<div class="alert alert-danger alert-dismissable"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>Error Charging the Credit-Card</div>');
            } else if (html === 'ok') {
                $('#' + what + 'Modal .btn').attr('disabled', false);
            } else {
                $('#' + what + 'Modal .btn').attr('disabled', false);
                theModal.html('<div class="alert alert-danger alert-dismissable"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>' + html + '</div>');
            }
        },
        error: function () {
            $('#' + what + 'Modal .btn').attr('disabled', false);
...
function get_crud_url() {
    ...
    if (crudSearchTerms.value.length > 0) url = url + '&search=' + JSON.stringify(crudSearchTerms);
    return url;
}
...
function crud_update_pager() {
    ...
    if (crudPage.value > 1) jQuery('#crud-pager-prev').removeClass('disabled');
    else jQuery('#crud-pager-prev').addClass('disabled');
    if (crudPage.value < crudTotalPages.value) jQuery('#crud-pager-next').removeClass('disabled');
    else jQuery('#crud-pager-next').addClass('disabled');
    pageLinks[0] = 1;
...
function crud_update_sort(that) {
    ...
    obj.find('i')
        ...
        .addClass('fa-sort-' + crudOrderDir.value);
    crud_load_page();
}
...
function crud_setup_pager_binds() {
    ...
    jQuery('#crud-pager-prev a').on('click', function (event) {
        ...
        if (crudPageOffset.value < 0) crudPageOffset.value = 0;
        crud_load_page();
    });
    jQuery('#crud-pager-next a').on('click', function (event) {
        event.preventDefault();
        crudPageOffset.value = crudPageOffset.value + crudPageLimit.value;
        if (crudPageOffset.value / crudPageLimit.value + 1 > crudTotalPages.value) crudPageOffset.value = (crudTotalPages.value - 1) * crudPageLimit.value;
        crud_load_page();
    });
}
...
<template>
    ...
    <div class="row">
        <div class="col-md-12">
            <div v-if="module && module === 'backups'" class="alert alert-default">(For pricing and more information: <a style="color: #004085" target="_blank" href="https://www.interserver.net/storage/">https://www.interserver.net/storage/</a>)</div>
            <div class="card">
                <div class="card-header text-right">
                    <div class="row float-right">
                        <div v-if="totalPages > 1" id="search_btns" class="col-md-auto printer-hidden pl-2 text-right">
                        ...
                        <div v-if="printButton || exportButton" id="print_expo_btns" class="col-md-auto export printer-hidden float-right pl-2">
                            <div class="btn-group">
                                <button v-if="printButton" class="btn btn-sm btn-secondary" type="button" title="Print" @click.prevent="crudPrint"><i class="fa fa-print crud-icon"></i>Print</button>
                                ...
                                <ul v-if="exportButton" class="dropdown-menu" role="menu">
                                    <li v-for="(formatData, ext) in exportFormats" :key="ext" role="presentation" :data-type="ext">
                                        <a href="#" data-container="body" :title="formatData.name" @click.prevent="crudExport($event, ext)"> <img :src="`/images/crud/${ext}.png`" alt="" /> {{ ext.toUpperCase() }} </a>
                                    </li>
                                </ul>
                            </div>
                        </div>
                ...
                <div class="card-body">
                    <div id="crud" class="crud">
                        <div class="row">
                            <div class="col-md-12">
                                <div class="table">
                                    <table id="crud-table" class="crud-table table-bordred table-striped table-hover table-sm table">
                                        <thead>
                                            <tr>
                                        ...
                                        <tbody>
                                            <tr v-for="(row, rowIndex) in tableRows" :key="rowIndex" :class="row.rowopts">
                                                ...
                                                <td v-for="(col, colIndex) in row" :key="colIndex" :colspan="col.colspan" :bgcolor="col.colbgcolor" :style="{ textAlign: col.colalign }" :class="col.colopts">
                                                    <template v-if="col in labelRep">
                                                        <span class="label label-sm" :class="'label-' + labelRep[col]">{{ col }}</span></template
                                                    >
