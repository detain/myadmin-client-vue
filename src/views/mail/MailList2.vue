...
origData.value = [
    {mail_id: "376503", mail_hostname: "hostingenuity.com", mail_expire_date: "2022-02-09 16:20:25", cost: "12.00", mail_status: "active"},
    {mail_id: "592337", mail_hostname: "detain.dev", mail_expire_date: "2023-08-14 00:59:38", cost: "18.00", mail_status: "active"},
    {mail_id: "418295", mail_hostname: "unixsrv10.com", mail_expire_date: "", cost: "11.00", mail_status: "canceled"},
    {mail_id: "408615", mail_hostname: "kirais.art", mail_expire_date: "", cost: "47.00", mail_status: "pending"},
    {mail_id: "408918", mail_hostname: "kiraart.bet", mail_expire_date: "", cost: "18.00", mail_status: "pending"}
];

const filteredData = computed(() => {
  const activeMail = [];
  origData.value.forEach(row => {
      row.link = '<router-link to="\'view_mail?id=' + row.mail_id + '\'" class="btn btn-primary btn-xs printer-hidden"><i class="fa fa-fw fa-cog"></i></router-link>';
      row.link = row.mail_id;
      row.hostname = '<router-link to="\'view_mail?id=' + row.mail_id + '\'">' + row.mail_hostname + '</router-link>';
      if (limitStatus.value == 'all' || limitStatusMap[limitStatus.value].includes(row.mail_status)) {
        activeMail.push(row);
      }
  });
  return activeMail;
