...
origData.value = [
    {backup_id: "376503", backup_hostname: "hostingenuity.com", backup_expire_date: "2022-02-09 16:20:25", cost: "12.00", backup_status: "active"},
    {backup_id: "592337", backup_hostname: "detain.dev", backup_expire_date: "2023-08-14 00:59:38", cost: "18.00", backup_status: "active"},
    {backup_id: "418295", backup_hostname: "unixsrv10.com", backup_expire_date: "", cost: "11.00", backup_status: "canceled"},
    {backup_id: "408615", backup_hostname: "kirais.art", backup_expire_date: "", cost: "47.00", backup_status: "pending"},
    {backup_id: "408918", backup_hostname: "kiraart.bet", backup_expire_date: "", cost: "18.00", backup_status: "pending"}
];

const filteredData = computed(() => {
  const activeBackups = [];
  origData.value.forEach(row => {
      row.link = '<router-link to="\'view_backup?id=' + row.backup_id + '\'" class="btn btn-primary btn-xs printer-hidden"><i class="fa fa-fw fa-cog"></i></router-link>';
      row.link = row.backup_id;
      row.hostname = '<router-link to="\'view_backup?id=' + row.backup_id + '\'">' + row.backup_hostname + '</router-link>';
      if (limitStatus.value == 'all' || limitStatusMap[limitStatus.value].includes(row.backup_status)) {
        activeBackups.push(row);
      }
  });
