...
const loadBackups = async (data) => {
    try {
        const response = await fetchWrapper.get('https://mystage.interserver.net/apiv2/backups_list');
      ...
      <div class="card-header text-right">
        <div class="row float-right">
          <div id="header_btns" class="col-md-auto printer-hidden text-right pl-2">
            <div class="btn-group">
              <router-link class='btn btn-primary btn-sm printer-hidden' to='backups/order' data-toggle="tooltip" title="Order Backup Registrations"><i class='fa fa-shopping-cart'></i> Order</router-link>
            </div>
          </div>
