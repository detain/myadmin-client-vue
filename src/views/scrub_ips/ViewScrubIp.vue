<template>
    <div class="row mt-2">
        <Package 
            :pkg
            :inv-next-date = billingDetails.service_next_invoice_date
            :host= serviceInfo.scrub_ip_ip 
        />
        <Billing 
            :service-status=serviceInfo.scrub_ip_status
            :currency-symbol=billingDetails.service_currency_symbol 
            :cost=cost
            :frequency=billingDetails.service_frequency
        />
        <InfoBox
            :heading="'Current IP'"
            :key1="'IP Address'"
            :val1="serviceInfo.scrub_ip_ip"
            :icon-class="'fas fa-info-circle'"
            :icon-name="' '"
            :footer-key="'IP from service'"
            :footer-val="footer_val ? footer_val : 'N/A'"
            :bg-class="'bg-info'"
        />
    </div>
				...
				<div class="card-header">
					<div class="p-1">
						<h3 class="card-title py-2">
							<i class="fa fa-plug">&nbsp;</i> {{ extraInfoTables[module]?.title }}
                        </h3>
...
<script setup lang="ts">
    ...
    const { loading, error, serviceInfo, clientLinks, billingDetails, custCurrency, custCurrencySymbol, 
        serviceExtra, extraInfoTables, serviceType, pkg, linkDisplay, firewallRules, filterRules } = storeToRefs(scrubIpStore);
