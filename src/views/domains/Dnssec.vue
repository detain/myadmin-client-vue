    ...
    <div class="col-md-12">
      <div class="card">
        <div class="card-header border-0">
          <a style="float: inline-start;" :href="'view_domain?id=' + id" class="btn-outline-custom py-1 px-2"
            data-toggle="tooltip" title="Go Back">
            <i class="fas fa-arrow-left text-sm"></i>&nbsp;Back
          </a>
          <h3 class="card-title mt-1 ml-2"><i class="fas fa-lock">&nbsp;</i>DNSSEC Infomation</h3>
          <div class="card-tools m-0">
            <a id="add-new" class="btn btn-custom py-2 text-sm px-3" href="javascript:void(0);" @click="showAddDNSContent">
              <i class="fa fa-plus-circle">&nbsp;</i>Add New Record
            </a>
            <template v-if="dnssec_records && dnssec_records.length">
              <form id="removethem" method="POST" :action="'view_domain?id=' + domain_id + '&link=dnssec'"
                class="d-inline">
                <input type="hidden" name="csrf_token" :value="csrf_token">
                <input type="hidden" name="action" value="delete">
                <button type="submit" class="btn btn-sm bg-gradient-red text-white"
                  @click.prevent="confirmDialog"><i class="fa fa-times-circle">&nbsp;</i>Remove All DNSSEC Records</button>
              </form>
            </template>
          </div>
        </div>
        <div class="card-body row justify-content-center">
          ...
          <template v-else>
            <h5 class="text-orange text-center w-100 tex-red py-2 b-radius text-bold">No DNSSEC Records found.</h5>
          </template>

          <div class="card form-gray col-md-10 mt-5" id="add-dns-content" v-show="showAddDNSContent">
            <div class="card-header p-3">
              <h3 class="card-title text-xl text-bold text-center" style="width: 95%;"> Add DNSSEC</h3>
              <div class="card-tools pt-2 m-0">
                <button type="button" class="btn btn-tool" data-card-widget="remove" @click="hideAddDNSContent">
                ...
                <div v-for="foo in 3" :key="foo">
                  <div class="form-group row">
                    ...
                    <div class="col-md-10 input-group">
                      <select :name="'digest_type[' + foo + ']'" class="form-control select2" style="width:100%;"
                        required>
                  ...
                  <div class="form-group row">
                    ...
                    <div class="col-sm-10 input-group">
                      <select :name="'algorithm[' + foo + ']'" class="form-control select2" style="width:100%;"
                        required>
                        <option value="">Select</option>
                  ...
                  <div class="form-group row">
                    ...
                    <div class="col-sm-10 input-group">
                      <input type="number" :placeholder="'Enter Key Tag ( Example: 2371 )'" :name="'key_tag[' + foo + ']'"
                  ...
                  <div class="form-group row">
                    ...
                    <div class="col-sm-10 input-group">
                      <textarea :placeholder="'Enter Value of Digest #' + (foo + 1) + ' ( Must not be greater than: ' +
                        (foo === 0 ? '40' : (foo === 1 ? '64' : '96')) + ' Characters )'" :id="'digest' + (foo + 1)"
                        class="form-control text-sm" :name="'digest[' + foo + ']'" rows="5" required></textarea>
                    </div>
                  </div>
                  <div class="form-group row">
                    <div class="col-sm-12">
                      <div class="text-right text-muted" :id="'display' + (foo + 1)">Characters left: <b>{{
                          foo === 0 ? '40' : (foo === 1 ? '64' : '96') }}</b></div>
                    </div>
                  </div>
                </div>
                <div class="form-group row">
                  ...
                  <div class="col-sm-10 input-group input-group-sm">
                    <input type="submit" name="Submit" value="Save" class="btn btn-custom pt-2 px-4"
                      id="button-id-signup" />
                  </div>
                </div>
              </form>
