  ...
  setup() {
    ...
    onMounted(() => {
      Swal.fire({
        ...
        confirmButtonText: `${
          funct.value === 'disableCancel' ? 'Disable & Cancel it.' : 'Enable Whois'
        }`,
        html: `
          <p>Your domain <span class="text-2lg">${domain.value}</span> Whois Privacy addon is ${
          funct.value === 'disableCancel' ? 'Enabled' : 'Disabled'
        }</p>
...
<template>
  <form
    id="whoisDisableForm"
    :action="`view_domain?id=${id}&link=whois`"
    method="POST"
  >
