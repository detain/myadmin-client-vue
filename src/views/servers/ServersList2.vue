...
origData.value = [
    {server_id: "376503", server_hostname: "hostingenuity.com", server_expire_date: "2022-02-09 16:20:25", cost: "12.00", server_status: "active"},
    {server_id: "592337", server_hostname: "detain.dev", server_expire_date: "2023-08-14 00:59:38", cost: "18.00", server_status: "active"},
    {server_id: "418295", server_hostname: "unixsrv10.com", server_expire_date: "", cost: "11.00", server_status: "canceled"},
    {server_id: "408615", server_hostname: "kirais.art", server_expire_date: "", cost: "47.00", server_status: "pending"},
    {server_id: "408918", server_hostname: "kiraart.bet", server_expire_date: "", cost: "18.00", server_status: "pending"}
];

const filteredData = computed(() => {
  const activeServers = [];
  origData.value.forEach(row => {
      row.link = '<router-link to="\'view_server?id=' + row.server_id + '\'" class="btn btn-primary btn-xs printer-hidden"><i class="fa fa-fw fa-cog"></i></router-link>';
      row.link = row.server_id;
      row.hostname = '<router-link to="\'view_server?id=' + row.server_id + '\'">' + row.server_hostname + '</router-link>';
      if (limitStatus.value == 'all' || limitStatusMap[limitStatus.value].includes(row.server_status)) {
        activeServers.push(row);
      }
  });
