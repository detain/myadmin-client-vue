    ...
    <div class="row mt-2">
        <div class="col-md-4">
            <div class="small-box bg-secondary">
                <div class="inner pt-3 pb-1 px-3">
                    ...
                    <p>Next Invoice Date: <b>{{ billingDetails.service_next_invoice_date || 'Un-billed' }}</b></p>
                </div>
        ...
        <div class="col-md-4">
            <div :class="`small-box bg-${serviceInfo.server_status === 'active' ? 'success' : serviceInfo.server_status === 'pending' ? 'orange' : (serviceInfo.server_status === 'expired' || serviceInfo.server_status === 'canceled') ? 'red' : ''}`">
                <div class="inner pt-3 pb-1 px-3">
                    <h3>Billing</h3>
                    <p class="py-3 my-2"><b>{{ billingDetails.service_currency_symbol }}{{ billingDetails.service_cost_info }}</b> billed: <b>{{ billingDetails.service_frequency }}</b></p>
                </div>
        ...
        <div class="col-md-4">
            <div class="small-box bg-info">
                <div class="inner pt-3 pb-1 px-3">
                    <h3>Order Info</h3>
                    <p class="py-3 my-2">Order ID: <b>{{ serviceInfo.server_id }}</b></p>
                </div>
                <div class="icon"><i class="fa fa-cart-plus"></i></div>
                <div class="small-box-footer">Ordered on: <b>{{ new Date(serviceInfo.server_date).toLocaleDateString('en-US', { day: 'numeric', month: 'long', year: 'numeric' }) }}</b></div>
            </div>
        </div>
    </div>
            ...
            <div class="card">
                <div class="card-header">
                    <div class="p-1">
                        ...
                        <div class="card-tools float-right pt-1 pl-3"><button type="button" class="btn btn-tool" data-card-widget="collapse"><i class="fas fa-minus" aria-hidden="true"></i></button></div>
                    </div>
                </div>
                <div class="card-body" style="height: 270px;">
                    <div class="row">
                        ...
                        <div class="col-md-12 text-center pt-2 pr-4 mr-3"></div>
                        <div class="row">
        ...
        <div :class="`${ipmiLease && ipmiLease !== false && ipmiLease.authenticated == true ? 'col-md-8' : 'col-md-4'}`">
            <div class="card">
                ...
                <div class="card-body text-center">
                    <router-link v-for="(clientLink, index) in clientLinks" :key="index" :to="'/servers/'+id+'/'+clientLink.link" class="btn btn-app mb-3" :title="clientLink.help_text" data-toggle="tooltip"><i :class="clientLink.icon" aria-hidden="true">{{ clientLink.icon_text }}</i>{{ clientLink.label }}</router-link>
                </div>
            </div>
        </div>
        <div class="col-md-8">
            <div class="card">
                ...
                <div class="card-body" style="overflow: scroll;">
                    <table class="table table-sm table-bordered" v-if="assets">
                        <thead>
                            <tr>
                                <th>ID</th>
                                <th>Hostname</th>
