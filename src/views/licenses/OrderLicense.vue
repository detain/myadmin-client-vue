        ...
        <div class="row row-cols-1 row-cols-md-2 row-cols-lg-3 g-3">
            <div v-for="(details, key) in getLicenses" :key="key" class="col"><div class="card h-100">
                ...
                <div class="card-body text-center" style="height: 270px">
                    ...
                    <p class="card-text text-start text-sm">{{ details.description }}</p>
                ...
                <div class="card">
                    <div class="card-header">
                        <div class="p-1">
                            ...
                            <div class="card-tools float-end">
                                <router-link to="/licenses/order" class="btn btn-custom text-sm" data-bs-toggle="tooltip" title="Go Back" style="position: relative; top: 5px"><i class="fas fa-arrow-left"></i>&nbsp;Back&nbsp;&nbsp;</router-link>
                            </div>
                        </div>
                    </div>
                    <div class="card-body">
                        <form id="license_form" method="post" class="license_form_init" @submit.prevent="submitForm">
                            <div class="mb-3 row">
                                ...
                                <div class="col-sm-9 input-group">
                                    <div v-for="(package_details, id) in getServiceTypes" :key="id" class="mb-3 w-100">
                                        <div class="form-check d-inline">
                                            <input :id="package_details.services_name" v-model="packageId" type="radio" class="form-check-input" name="package" :value="id" />
                                            <label class="form-label form-check-label more-info fw-normal" :for="package_details.services_name">{{ package_details.services_name }}</label>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            <div class="mb-3 row">
                                <label class="col-sm-3 col-form-label text-end">IP Address<span class="text-danger"> *</span></label>
                                <div class="col-sm-9 input-group">
                                    <input v-model="ip" type="text" name="ip" class="form-control form-control-sm" placeholder="IP Address" required @change="updatePrice()" />
                                </div>
                            </div>
                            <div v-if="getLicenses[catTag]?.name !== 'cPanel'" id="coupon_row" class="mb-3 row">
                                ...
                                <div class="col-md-9">
                                    <button class="btn bg-secondary btn-sm me-2 mt-1" @click="checkAvailability()">Check availability</button>
                ...
                <div class="card">
                    ...
                    <div class="card-body text-md">
                        <div class="row mb-3">
                            ...
                            <div class="col text-bold text-end">1 Month</div>
                        </div>
                        <div class="row mb-3">
                            <div id="hostname_display" class="col-md-6">Package Cost</div>
                            <div class="col package_cost text-bold text-end">{{ currencySymbol }}{{ serviceTypes[Number(packageId)]?.services_cost }}</div>
                        </div>
                        ...
                        <div class="row mb-3">
                            ...
                            <div id="totalprice" class="col text-bold total_cost text-end text-lg">{{ currencySymbol }}{{ serviceTypes[Number(packageId)]?.services_cost }}</div>
                        </div>
                    </div>
                </div>
                        ...
                        <form ref="licenseForm" method="post" class="license_form_confirm" @submit.prevent="submitLicenseForm">
                            <table class="table-sm table-bordered table">
                                <thead>
                                    <tr>
                                        <th>
                                            <div class="text-md float-start" style="position: relative; top: 5px">{{ serviceTypes[Number(packageId)].services_name }}</div>
                            ...
                            <div class="row">
                                <div class="controls col-md-12 text-center">
                                    <router-link :to="`/licenses/order/${catTag}`" class="btn btn-custom btn-sm px-3 py-2 me-2" data-bs-toggle="tooltip" title="Go Back" style=""><i class="fas fa-arrow-left"></i>&nbsp;Go Back&nbsp;&nbsp;</router-link>
