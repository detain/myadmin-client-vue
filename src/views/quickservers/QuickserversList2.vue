...
origData.value = [
    {qs_id: "376503", qs_hostname: "hostingenuity.com", qs_expire_date: "2022-02-09 16:20:25", cost: "12.00", qs_status: "active"},
    {qs_id: "592337", qs_hostname: "detain.dev", qs_expire_date: "2023-08-14 00:59:38", cost: "18.00", qs_status: "active"},
    {qs_id: "418295", qs_hostname: "unixsrv10.com", qs_expire_date: "", cost: "11.00", qs_status: "canceled"},
    {qs_id: "408615", qs_hostname: "kirais.art", qs_expire_date: "", cost: "47.00", qs_status: "pending"},
    {qs_id: "408918", qs_hostname: "kiraart.bet", qs_expire_date: "", cost: "18.00", qs_status: "pending"}
];

const filteredData = computed(() => {
  const activeQuickservers = [];
  origData.value.forEach(row => {
      row.link = '<router-link to="\'view_qs?id=' + row.qs_id + '\'" class="btn btn-primary btn-xs printer-hidden"><i class="fa fa-fw fa-cog"></i></router-link>';
      row.link = row.qs_id;
      row.hostname = '<router-link to="\'view_qs?id=' + row.qs_id + '\'">' + row.qs_hostname + '</router-link>';
      if (limitStatus.value == 'all' || limitStatusMap[limitStatus.value].includes(row.qs_status)) {
        activeQuickservers.push(row);
      }
  });
  return activeQuickservers;
})
