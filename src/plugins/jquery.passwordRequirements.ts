 ...
 */
($.fn as any).passwordRequirements = function (
  ...
  return this.each(function (this: HTMLElement) {
    ...
    o.infoMessage =
      `The minimum password length is ${o.numCharacters} characters and must contain ` +
      `at least 1 lowercase letter, 1 capital letter, 1 number, and 1 special character.`;
    ...
    const messageDiv =
      `<div id="pr-box"><i></i>` +
      `<div id="pr-box-inner"><p>${o.infoMessage}</p>` +
      `<ul>${numCharactersUI}${useLowercaseUI}${matchPassUI}${useUppercaseUI}${useNumbersUI}${useSpecialUI}</ul>` +
      `</div></div>`;
