...
export const fetchWrapper = {
    ...
    get: request('GET'),
    getNoLogout: requestNoLogout('GET'),
...
function requestNoLogout(method: string) {
    return (url: string) => {
        console.log(`sending a ${method} request to ${url}`);
        const requestOptions = {
            method,
...
function authHeader(url: string): any {
    ...
    const { apiKey, sessionId } = useAuthStore();
...
async function handleResponseNoLogout(response: any) {
    const isJson = response.headers?.get('content-type')?.includes('application/json');
    const data = isJson ? await response.json() : null;
    // check for error response
    if (!response.ok) {
